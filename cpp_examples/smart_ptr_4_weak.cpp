/* std::weak_ptr:
    A non-owning smart pointer which can be initialized from a std::shared_ptr. 
    A std::weak_ptr cannot be derefenced directly, but has a member function lock() 
    which returns a suitable std::shared_ptr (within the scope of the call to lock()) which can be dereferenced
*/


/***** Use of std::shared_ptr may be unsuitable in this case becuase of the dependency cycle created
    When the containers AllClasses and AllPupils are destroyed or go out of scope, this 
    does not cause the destructors of the Class and Pupil objects to be called correctly; 
    this is caused by the semantics being wrong as a Class cannot “own” its Pupils if the Pupils also “own” the Class
...
int main() {
    ...
    auto add_to_class = [&](string_view c, string_view p) {
        ...
        if (iter_c != cend(AllClasses) && iter_p != cend(AllPupils)) {
            // Check to ensure the student is not already registed to the class
            auto classes = (*iter_p)->classes;
            if (classes.size() == 0 || find_if(classes.begin(), classes.end(), 
                    [&](auto pcls){return pcls.lock()->subject == c;}) == classes.end()) {
    ...
    for (;;) {
        ...
        if (iter_p != cend(AllPupils)) {
            ...
            for (const auto& c : (*iter_p)->classes) {
                if (auto pc = c.lock(); pc) {          // weak_ptr must be converted to std::shared_ptr in order to access the referenced object
                    cout << pc->subject << ' ';
                }
            }
            cout << '\n';
